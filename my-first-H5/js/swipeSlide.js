...
;(function(win,$){
    ...
    var iosVersion = +ua.substring(ua.indexOf('OS')+2,ua.indexOf('like Mac OS X')).trim().replace(/_/, '.');
    var wifi = true;
    ...
    function fnTransition(me, dom, num){
        // 解决iOS8.1/8.4下闪退，降级处理
        if(iosVersion == 8.1 || iosVersion == 8.4){
            return;
        }
        dom.css({
    ...
    function fnLazyLoad(me, index){
        ...
        if($thisImg){
            $thisImg.each(function(i){
                ...
                var imgSrc2x = $this.attr('data-src2x');
                var imgSrc3x = $this.attr('data-src3x');
                if(wifi){
                    if(imgSrc3x && imgSrc2x){
                        $this.attr('srcset', imgSrc2x+' 2x,'+imgSrc3x+' 3x').attr('src', imgSrc2x);
                    }else if(imgSrc3x){
                        $this.attr('srcset', imgSrc+' 2x,'+imgSrc3x+' 3x').attr('src', imgSrc);
                    }else if(imgSrc2x){
                        $this.attr('src', imgSrc2x);
                    }else{
    ...
    function fnAutoSlide(me){
        if(me.opts.autoSwipe) fnStopSlide(me);
        if((me.opts.setSwipeTimes && me.opts.swipeTimes > 0) 
            || (!me.opts.setSwipeTimes && me.opts.swipeTimes === 0)){
